...
[Authorize]
public class UserFavoriteMenuAppService : TestWorkshopAppService, IUserFavoriteMenuAppService
{
    ...
    [Authorize(TestWorkshopPermissions.Menu.ManageUserFavorites)]
    public async virtual Task<UserFavoriteMenuDto> CreateAsync(Guid userId, UserFavoriteMenuCreateDto input)
    {
        ...
        {
            throw new BusinessException(TestWorkshopErrorCodes.UserDuplicateFavoriteMenu);
        }
    ...
    protected async virtual Task<UserFavoriteMenu> GetUserMenuAsync(Guid userId, Guid menuId)
    {
        ...
        return userFavoriteMenu ?? throw new BusinessException(TestWorkshopErrorCodes.UserFavoriteMenuNotFound);
    }
    ...
    protected virtual void UpdateByInput(UserFavoriteMenu userFavoriteMenu, UserFavoriteMenuCreateOrUpdateDto input)
    {
        ...
        {
            userFavoriteMenu.Color = Check.Length(input.Color, nameof(UserFavoriteMenuCreateOrUpdateDto.Color), TestWorkshopConsts.MaxLength64);
        }

        if (!string.Equals(userFavoriteMenu.AliasName, input.AliasName, StringComparison.CurrentCultureIgnoreCase))
        {
            userFavoriteMenu.AliasName = Check.Length(input.AliasName, nameof(UserFavoriteMenuCreateOrUpdateDto.AliasName), TestWorkshopConsts.MaxLength128);
        }

        if (!string.Equals(userFavoriteMenu.Icon, input.Icon, StringComparison.CurrentCultureIgnoreCase))
        {
            userFavoriteMenu.Icon = Check.Length(input.Icon, nameof(UserFavoriteMenuCreateOrUpdateDto.Icon), TestWorkshopConsts.MaxLength512);
        }
    }
}
