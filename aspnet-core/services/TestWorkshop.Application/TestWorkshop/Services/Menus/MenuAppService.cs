...
[Authorize]
public class MenuAppService : TestWorkshopAppService, IMenuAppService
{
    ...
    [Authorize(TestWorkshopPermissions.Menu.Default)]
    ...
    [Authorize(TestWorkshopPermissions.Menu.Create)]
    public async virtual Task<MenuDto> CreateAsync(MenuCreateDto input)
    {
        ...
        foreach (var dataItem in data.Items)
        {
            if (!input.Meta.TryGetValue(dataItem.Name, out object meta))
            {
                ...
                {
                    throw new BusinessException(TestWorkshopErrorCodes.MenuMissingMetadata)
    ...
    [Authorize(TestWorkshopPermissions.Menu.Update)]
    ...
    [Authorize(TestWorkshopPermissions.Menu.Delete)]
    public async virtual Task DeleteAsync(Guid id)
    {
        var childrens = await MenuRepository.GetChildrenAsync(id);
        if (childrens.Any())
        {
            throw new BusinessException(TestWorkshopErrorCodes.DeleteMenuHaveChildren);
        }
    ...
    [Authorize(TestWorkshopPermissions.Menu.ManageUsers)]
    ...
    [Authorize(TestWorkshopPermissions.Menu.ManageRoles)]
