...
export function useRolesApi() {
  ...
  function removeOrganizationUnitApi(id: string, ouId: string): Promise<void> {
    return request(
      `/api/system/identity/roles/${id}/organization-units/${ouId}`,
      {
        method: 'DELETE',
      },
    );
  ...
  /**
   * 获取所有角色
   * @returns 角色实体数据传输对象列表
   */
  function getAllListApi(): Promise<ListResultDto<IdentityRoleDto>> {
    return request<ListResultDto<IdentityRoleDto>>(
      `/api/system/identity/roles/all`,
      {
        method: 'GET',
      },
    );
  ...
  function createApi(input: IdentityRoleCreateDto): Promise<IdentityRoleDto> {
    return request<IdentityRoleDto>('/api/system/identity/roles', {
      data: input,
      method: 'POST',
    });
  }
  ...
  function deleteApi(id: string): Promise<void> {
    return request(`/api/system/identity/roles/${id}`, {
      method: 'DELETE',
    });
  }
  ...
  function getApi(id: string): Promise<IdentityRoleDto> {
    return request<IdentityRoleDto>(`/api/system/identity/roles/${id}`, {
      method: 'GET',
    });
  }
